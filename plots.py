...
def plot_state_time(time_arr):
    ...
    fig = go.Figure(go.Bar(x = values, y = labels, orientation='h', title = 'Time (in Years) Individual Spends in Each State'))
...
def plot_stage_time(total_time_arr):
    ...
    fig = go.Figure(go.Bar(x = values, y = labels, orientation='h', title = 'Time (in Years) Individual Spends in Each Victimization Stage'))
